...
Extracts values from JSON responses using JSONPath expressions. Used for correlation (passing data between requests).

```xml
<JSONPostProcessor guiclass="JSONPostProcessorGui" testclass="JSONPostProcessor"
                   testname="Extract user_id" enabled="true">
...
**Key Properties:**
- `JSONPostProcessor.referenceNames`: Variable name(s) to store extracted values
- `JSONPostProcessor.jsonPathExprs`: JSONPath expression(s) to extract values
- `JSONPostProcessor.match_numbers`: Which match to use
  - `1`: First match
  - `0`: Random match
  - `-1`: All matches (creates array)
- `JSONPostProcessor.defaultValues`: Default value if no match found

**Multiple Extractions:**
```xml
<JSONPostProcessor guiclass="JSONPostProcessorGui" testclass="JSONPostProcessor"
                   testname="Extract Multiple Values" enabled="true">
  <stringProp name="JSONPostProcessor.referenceNames">user_id;token;name</stringProp>
  <stringProp name="JSONPostProcessor.jsonPathExprs">$.id;$.auth.token;$.user.name</stringProp>
...
| `$.user.name` | Nested `name` in `user` object |
| `$.items[0].id` | First element's `id` in `items` array |
| `$.data[*].name` | All `name` fields in `data` array |
| `$..id` | All `id` fields at any level |

**Python Example:**
```python
def create_json_post_processor(variable_name, json_path, default="NOT_FOUND"):
...
Executes scripts (Groovy, JavaScript) after a sampler response. Used for complex extraction logic.

```xml
<JSR223PostProcessor guiclass="TestBeanGUI" testclass="JSR223PostProcessor"
                     testname="Post-Response Script" enabled="true">
...
**Key Properties:**
- `scriptLanguage`: Script language (groovy, javascript, jython, beanshell)
- `script`: Script content
- `cacheKey`: Cache compiled script for performance (recommended: true)

**When to Use:**
- **JSONPostProcessor**: Simple value extraction from JSON responses
- **JSR223PostProcessor**: Complex logic (conditionals, transformations, multiple sources)
